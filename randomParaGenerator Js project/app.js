...
function generate(){
  if(paraCount.value<1||paraCount.value>9||isNaN(paraCount.value)){
         const randomIndex=Math.floor(Math.random()*text.length);
         const paras=text[randomIndex];
         console.log(paras);
         outputArea.innerHTML=paras;
  }
  else{
    const randomArrayText=shuffle(text);
     const data=randomArrayText.slice(0,paraCount.value);
 ...
 outputArea.innerHTML=paras.join("");
  }
}
